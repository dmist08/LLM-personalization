"""
Fast Cold-Start inference — same batch-by-author approach.
"""
...
logging.basicConfig(level=logging.INFO,
    format="%(asctime)s | %(levelname)-7s | cs_fast | %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S")
...
def load_cs_vector(cold_start_dir, author_id):
...
def batch_generate_with_steering(model, tokenizer, articles, style_vector,
    ...
    inputs = tokenizer(prompts, return_tensors="pt", truncation=True,
                       max_length=512, padding=True, pad_to_multiple_of=8).to(device)
    ...
    try:
        ...
        results = []
        for out_ids, inp_ids in zip(out, inputs["input_ids"]):
            prompt_len = (inp_ids != tokenizer.pad_token_id).sum().item()
            text = tokenizer.decode(out_ids[prompt_len:], skip_special_tokens=True).strip()
            for stop in ["\n", "Article:", "Generate"]:
                if stop in text:
...
def main():
    ...
    cs_dir = args.cold_start_dir or ds["cold_start_dir"]
    ...
    for rec in records:
        by_author[str(rec[ds["id_field"]])].append(rec)

    log.info(f"Authors: {len(by_author)} | Records: {len(records)} | Batch: {args.batch_size}")
    ...
    for author_idx, (author_id, author_recs) in enumerate(by_author.items()):
        sv = load_cs_vector(cs_dir, author_id)
        if sv is None:
            skipped += len(author_recs)
            continue

        pending = [r for r in author_recs
        ...
        for batch_start in range(0, len(pending), args.batch_size):
            ...
            try:
                headlines = batch_generate_with_steering(
                    model, tokenizer, articles, sv, args.layer, args.alpha)
            except Exception as e:
                log.warning(f"Batch error {author_id}: {e}")
                headlines = [""] * len(batch)
                errors += len(batch)
        ...
        if (author_idx + 1) % 5 == 0:
            ...
            eta = (elapsed / (author_idx + 1)) * (len(by_author) - author_idx - 1) / 60
